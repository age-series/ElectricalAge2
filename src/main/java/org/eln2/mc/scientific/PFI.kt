...
val xcomDataset = Json.decodeFromString<XCOMDataset>(
    if(USE_GAME)
        getResourceString(Eln2.resource("datasets/xcom/pcs.json"))
    else
...
fun fetchElementNode(z: Int): ElementNode {
    ...
    return xcomDataset.nodes[(if(z < 10) "Z00$z" else { if(z < 100) "Z0$z" else "Z$z"})]!!
}
...
data class ElementNode(val absorptionEdge: XCOMElementAbsorptionEdgeNode, val entries: List<XCOMElementDataNode>) {
    ...
    @Transient
    val nuclearFieldPairProductionProcess = lnRemapPairProduction(orderedEntries.map { it.energy to it.pairAtom }, 1.022007e6)

    @Transient
    val electronFieldPairProductionProcess = lnRemapPairProduction(orderedEntries.map { it.energy to it.pairElectron }, 2.044014e6)
    ...
    companion object {
        ...
        private fun lnRemapPairProduction(entries: List<Pair<Double, Double>>, pairProductionThreshold: Double): InterpolationFunctionDual<Quantity<Energy>, Double, Dual> {
...
fun computePhotonCrossSectionBarnsDual(element: ChemicalElement, e: Quantity<Energy>, n: Int = 1): Dual {
    ...
    return node.coherentProcess.evaluateDual(e, n) +
        ...
        node.photoelectricProcess.evaluateDual(e, n) + // without edge energy phenomena, already a bit overkill for a quick game of Minecraft
