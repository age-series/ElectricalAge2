...
object Scheduler {
    ...
    private class WorkQueue {
        ...
        fun scheduleOnce(timestamp: Long, item: () -> Unit) = queue.add(WorkItem(
            timestamp,
            item,
    ...
    private fun getEventQueue(listener: EventListener) = eventQueues[listener] ?: error("Could not find event queue for $listener")
...
@ServerOnly fun schedulePre(countdown: Int, item: () -> Unit) = Scheduler.scheduleWork(countdown, item, Phase.START)
@ServerOnly fun schedulePost(countdown: Int, item: () -> Unit) = Scheduler.scheduleWork(countdown, item, Phase.END)
@ServerOnly fun runPre(block: () -> Unit) = Scheduler.scheduleWork(0, block, Phase.START)
@ServerOnly fun runPost(block: () -> Unit) = Scheduler.scheduleWork(0, block, Phase.END)
@ServerOnly fun periodicPre(interval: Int, item: () -> Boolean) = Scheduler.scheduleWorkPeriodic(interval, item, Phase.START)
@ServerOnly fun periodicPost(interval: Int, item: () -> Boolean) = Scheduler.scheduleWorkPeriodic(interval, item, Phase.END)
