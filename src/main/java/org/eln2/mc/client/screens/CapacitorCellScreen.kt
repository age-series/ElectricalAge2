...
class CapacitorCellScreen(private val container: CapacitorCellContainer, inv: Inventory, name: Component) :
    AbstractContainerScreen<CapacitorCellContainer>(container, inv, name) {

    companion object {
        private val guiTexture: ResourceLocation = ResourceLocation(Eln2.MODID, "textures/gui/capacitor_gui.png")
    ...
    override fun init() {
        ...
        textbox.value = capacitance.toString()
        saveButton = Button(relX + buttonX, relY + buttonY, buttonW, buttonH, TranslatableComponent("gui.eln2.save")) {
            try {
                val capcitance = textbox.value.toDouble()
                this.container.value = capcitance
                Networking.sendToServer(SingleDoubleElementGuiUpdatePacket(capcitance, container.pos))
                this.onClose()
    ...
    override fun render(pPoseStack: PoseStack, pMouseX: Int, pMouseY: Int, pPartialTick: Float) {
        ...
        if (container.value != capacitance) {
            capacitance = container.value
            textbox.value = "%.6f".format(capacitance)
        }
