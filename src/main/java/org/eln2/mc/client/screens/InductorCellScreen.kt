...
import org.eln2.mc.common.containers.InductorCellContainer
import org.eln2.mc.common.network.Networking
import org.eln2.mc.common.network.clientToServer.SingleDoubleElementGuiUpdatePacket
import java.awt.Color

class InductorCellScreen(private val container: InductorCellContainer, inv: Inventory, name: Component) :
    AbstractContainerScreen<InductorCellContainer>(container, inv, name) {

    companion object {
        private val guiTexture: ResourceLocation = ResourceLocation(Eln2.MODID, "textures/gui/inductor_gui.png")
    ...
    override fun init() {
        ...
        textbox.value = inductance.toString()
        saveButton = Button(relX + buttonX, relY + buttonY, buttonW, buttonH, TranslatableComponent("gui.eln2.save")) {
            try {
                val inductance = textbox.value.toDouble()
                this.container.value = inductance
                Networking.sendToServer(SingleDoubleElementGuiUpdatePacket(inductance, container.pos))
                this.onClose()
    ...
    override fun render(pPoseStack: PoseStack, pMouseX: Int, pMouseY: Int, pPartialTick: Float) {
        ...
        if (container.value != inductance) {
            inductance = container.value
            textbox.value = "%.6f".format(inductance)
        }
