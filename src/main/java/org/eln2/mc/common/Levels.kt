...
data class GTBSUpdateEvent(
    ...
    fun actual() = BlockEventFrame(listOf(
...
class BlockStreamProvider(val level: LevelReader) {
    ...
    private fun createLargeFrameBuffer(iSize: Int) = ArrayList<LocatedBlockState>(iSize) // maybe see if pools would help
    ...
    private data class Source(override val chunkPos: ChunkPos, val id: Int, val manager: BlockStreamProvider) : BlockEventSource {
        private val destroyed = AtomicBoolean()

        override val gtEvents = EventManager(setOf(
