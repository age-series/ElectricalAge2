...
class BVHIncrementalSAH<D, B : BoundingBox<B>> : MutableBVH<D, B> {
    ...
    private fun treeError() { error("Invalid tree") }
...
class DynamicBVHIncrementalSAH<D, B : BoundingBox<B>>(val adjustBox: (B) -> B) : ReinsertDynamicBVH<D, B> {
    private data class Obj<D, B : BoundingBox<B>>(
        val instance: D,
        val originalBox: B,
    )

    private val bvh = BVHIncrementalSAH<Obj<D, B>, B>()
    ...
    override fun query(test: BVHTest<D, B>, user: (D) -> Boolean) = bvh.query({ data: Obj<D, B>?, box: B, isLeaf: Boolean ->
